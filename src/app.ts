 ...
 */
async function playAudio({ text, audioBase64 }: Sentence, repeatCount: number = 1, pause:number = 1000) {
  console.log('Playing audio for sentence:', text);
  // Display the text in the UI
  geminiOutput.textContent = `${geminiOutput.textContent}\n${text}`;
  if (!audioBase64) return alert('No audio data received or MIME type missing.');
