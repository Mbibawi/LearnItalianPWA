...
async function askGemini() {
  ...
  if (!userQuery) return console.log('Invalid user query');
  ...
  async function submitUserQuery(accessToken:string, userQuery:string):Promise<string> {
    const data = await fetchWithOAuth(
          accessToken,
          apiUrl,
          {
...
async function fetchWithOAuth(accessToken:string, url: string, body: object): Promise<any> {
...
function handleOAuthCallback(): { accessToken: string | null; isValidState: boolean } {
...
async function translateWithGeminiApi(accessToken: string, text: string, targetLanguage: string): Promise<string> {
...
const italianVoiceName = voices.find(voice => voice.lang === 'it-IT' && voice.name.includes('Female'))?.name;
...
async function translateText(accessToken:string, textToTranslate: string, targetLanguage:string): Promise<string | void> {
